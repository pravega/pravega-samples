...
so that we don't need to manually deal with many connection parameters to create tables.
...
- **DataGen**: the data generator. By default, it will send 1000 events per second, lasting for about 3 hours. You can also
  modify the `speedup` parameter in `docker-compose.yml` to adjust the data generating speed. It will first register the
  schema to Pravega Schema Registry service and then send the generated data to Pravega cluster automatically so that 
  ...
  We use Avro as the format for serialization.
...
which will construct a new table format `pravega-registry` that can describe the mapping between Pravega stream and Flink tables.
